...
class Solution:
    ...
    def twoSum2PointersForSortedNums(self, nums, target, preFixValue1,preFixValue2):
        ans_list = list()
        ...
        while(leftIndex < rightIndex and rightIndex > 0):
            ...
            if sValue == target:
                ans_list.append([preFixValue1,preFixValue2, nums[leftIndex], nums[rightIndex]])
                # leftIndex += 1
                # rightIndex -= 1
                tLeft = leftIndex
    ...
    def threeSum(self, nums: List[int], target, preFixValue) -> List[int]:
        ans_list = list()
        nums_len = len(nums)
        index = 0
        while (index < nums_len - 1):
            rem_target = target - nums[index]
            temp_ans_list = self.twoSum2PointersForSortedNums(nums[index + 1:], rem_target,preFixValue, nums[index])
    ...
    def fourSum(self, nums: List[int], target: int) -> List[int]:
        ...
        while (index < nums_len - 1):
            rem_target = target - nums[index]
            temp_ans_list = self.threeSum(nums[index + 1:], rem_target, nums[index])
